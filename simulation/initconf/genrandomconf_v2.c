...
int i,j,k,l,statenum,collisionbool,*trapcount,seed,pointnum;
double **coor,radius,len,*vector,distsq,*center,chromatinlen,**pointcoor,gyr;
FILE *outputfile;
...
//printf("Input the number of points for the free molecules:\n");
//scanf("%d",&pointnum);
...
//pointcoor = doublematrix(pointnum,DIM);
...
for(i=0;i<statenum;i++)
 for(j=0;j<DIM;j++)
  {
  coor[i][j] -= center[j];
  gyr += coor[i][j]*coor[i][j];
  }

gyr= sqrt(gyr/statenum);

//for(i=0;i<pointnum;i++)
// {
// for(j=0;j<DIM;j++)
//  {
//  pointcoor[i][j] = 4*gyr*(randomdouble()-0.5);
//  }
// }
...
// fprintf(outputfile,"1 ");
 for(j=0;j<DIM;j++)
  fprintf(outputfile,"%lf ",coor[i][j]);
 fprintf(outputfile,"\n");
 }
...
// for(j=0;j<DIM;j++)
//  fprintf(outputfile,"%lf ",pointcoor[i][j]);
//// printf("check %d\n",i);
// fprintf(outputfile,"\n");
//// printf("check %d\n",i);
// }
