...
double getavevalue(double *distlist,int distlen)
 {
 int i,j,k;
 double ave;
...
//  medianvalue = getmedianvalue(distlist,distlen);
  medianvalue = getavevalue(distlist,distlen);
...
for(i=0;i<intervalnum;i++)
 ...
 for(j=0;j<intervalnum;j++)
  ...
  if(i==j)
   ...
   fprintf(outputfile,"%lf ",distmatrix[i][j]);
...
//   fprintf(outputfile,"%lf ",-log(distmatrix[i][j]/avedist[j-i-1])/log(10));
 ...
 fprintf(outputfile,"\n");
  ...
  printf("%d %lf\n",i,(distmatrix[i][i+1]+distmatrix[i][i-1])/2);
