...
void updatetargetpeakassign(int *targetpeakassign,int statenum,int currentid,double scorebar,double *score)
 {
 int i,j,k,l,id;
 double currentscore,maxdiff,endbool,diff;
 
 currentscore = score[currentid];

 maxdiff = 0;
// printf("%d %d\n",currentid,targetpeakassign[currentid]);

 if(targetpeakassign[currentid] != -1)
  {
  endbool=0;
  maxdiff = 0;
  for(i=currentid-1;(i>=0)&&(endbool==0);i--)
   {
   if(targetpeakassign[i]==-1)
    {
    diff = score[currentid]-score[i];
 ...
 if(targetpeakassign[currentid] != -1)
  ...
  endbool=0;
  maxdiff = 0;
  for(i=currentid+1;(i<statenum)&&(endbool==0);i++)
   {
   if(targetpeakassign[i]==-1)
    {
    diff = score[currentid]-score[i];
    ...
    {
    if(score[i] < score[currentid])
     {
     if(maxdiff < scorebar)
      targetpeakassign[i] = -1;
     }
    else
     endbool = 1;
    }
...
int i,j,statenum,*peakidlist,peaknum,genedist,*peakassign,*assign,*assignbool,*targetpeakassign;
double *avelist,*sigmalist,scorebar;
...
printf("Input the barrier for two neighbor peaks:\n");
scanf("%lf",&scorebar);
...
targetpeakassign = intarray(statenum);
...
for(i=0;i<peaknum;i++)
 targetpeakassign[peakidlist[i]] = 1;

for(i=0;i<peaknum;i++)
 {
 updatetargetpeakassign(targetpeakassign,statenum,peakidlist[i],scorebar,avelist);
 }
