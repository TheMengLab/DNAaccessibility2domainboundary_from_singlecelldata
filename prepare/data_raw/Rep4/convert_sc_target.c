...
void main()
{
char idlistfilename[N],pairfilename[N],singlefilename[N],outputfilename[N],**idlist,**pairlist,**charindexlist,tempcharindex[50],line1[LINELEN],line2[LINELEN],line3[LINELEN],line4[LINELEN],line[LINELEN];
...
for(i=0;i<pairnum;i++)
 {
 fscanf(pairfile,"%s",line1);
 fscanf(pairfile,"%s",line2);
 fscanf(pairfile,"%s",line3);
 fscanf(pairfile,"%s",line);
 if(i%100000==0)
  printf("%d %s\n",i,line);

 id = instringlist(idlist,idnum,line);
// id = getinstringlist(idlist,idnum,line,charindexlist,charindexlistlen,tempcharindex,charindexrange);
// if(id != -1)
//  count[id] ++;
 fscanf(pairfile,"%s",line4);
 if(id != -1)
  fprintf(outputfile,"%s %s %s %s %s\n",line1,line2,line3,line,line4);
